...
type MemcachesStatus struct {
	// Represents the observations of a Memcaches's current state.
	// Memcaches.status.conditions.type are: "Available", "Progressing", and "Degraded"
	// Memcaches.status.conditions.status are one of True, False, Unknown.
	// Memcaches.status.conditions.reason the value should be a CamelCase string and producers of specific
	// condition types may define expected values and meanings for this field, and whether the values
	// are considered a guaranteed API.
	// Memcaches.status.conditions.Message is a human readable message indicating details about the transition.
...
func init() {
	SchemeBuilder.Register(&Memcaches{}, &MemcachesList{})
}
